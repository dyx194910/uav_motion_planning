...
void initDataset() {
  ...
  std::string filename = dataset_folder_ + "/rrt_star_results_" + timestamp + ".csv";
...
void GoalCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  ...
  if (success == 1) {
    ...
    recordResult("rrt_star_hill", start_pt, end_pt, true, search_time, 0, cost);

    visualization_msgs::Marker path_marker;
    path_marker.header.frame_id = "world";
    path_marker.header.stamp = ros::Time::now();
    path_marker.ns = "rrt_star/path";
    path_marker.id = 0;
    ...
    recordResult("rrt_star_hill", start_pt, end_pt, false, search_time, 0, -1.0);
