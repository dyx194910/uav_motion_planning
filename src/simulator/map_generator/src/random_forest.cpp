...
double x_l_, x_h_, y_l_, y_h_, z_l_, z_h_, w_l_, w_h_, h_l_, h_h_, radius_l_,
    radius_h_, theta_, init_radius_;
      ...
      for (int ifx = -0; ifx <= 0; ifx++) {
        for (int ify = -0; ify <= 0; ify++) {
          for (int ifz = -0; ifz <= 0; ifz++) {
            cpt_if = cpt + Eigen::Vector3d(ifx * resolution_, ify * resolution_,
                                           ifz * resolution_);
...
void MapGenerate() {
  switch (fix_map_type_) {
    ...
    case 2: {
      GenerateWall(wall_x_ - 0.25, wall_x_ + 0.25, wall_y_ + wall_w_ / 2.0,
                   wall_y_ + 20.0, -0.5, 4.0);
      GenerateWall(wall_x_ - 0.25, wall_x_ + 0.25, wall_y_ - 20.0,
                   wall_y_ - wall_w_ / 2.0, -0.5, 4.0);
      break;
    }
    case 3: {
    ...
    case 4: {
      ...
      GenerateWall(0.0 - r1 - width_2, 0.0 + r1 + width_2, -0.6 - width_2,
                   -0.6 + width_2, 0, 0.9 - r1);
      // GenerateWall(0.0 + r1 - width_2, 0.0 + r1 + width_2, -0.6 - width_2,
      // -0.6 + width_2, 0, 0.9 + r1); GenerateWall(0.0 - r1 - width_2, 0.0 - r1
      // + width_2, -0.6 - width_2, -0.6 + width_2, 0, 0.9 + r1);

      GenerateCircle(0.0, 2.4, 1.2, r1, M_PI_2);
      GenerateCircle(0.0, 2.4, 1.2, r2, M_PI_2);
      GenerateWall(0.0 - r1 - width_2, 0.0 + r1 + width_2, 2.4 - width_2,
                   2.4 + width_2, 0, 1.1 - r1);
      // GenerateWall(0.0 + r1 - width_2, 0.0 + r1 + width_2, 2.4 - width_2, 2.4
      // + width_2, 0, 1.1 + r1); GenerateWall(0.0 - r1 - width_2, 0.0 - r1 +
      // width_2, 2.4 - width_2, 2.4 + width_2, 0, 1.1 + r1);

      width_2 = 0.1;
      GenerateWall(0.6 - width_2, 0.6 + width_2, 0.6 - width_2, 0.6 + width_2,
                   0, 2.0);
      GenerateWall(-0.6 - width_2, -0.6 + width_2, 0.6 - width_2, 0.6 + width_2,
                   0, 2.0);
      GenerateWall(0.0 - width_2, 0.0 + width_2, 1.2 - width_2, 1.2 + width_2,
                   0, 2.0);
      break;
    }
    case 5: {
